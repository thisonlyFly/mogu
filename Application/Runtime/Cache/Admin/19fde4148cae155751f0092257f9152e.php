...
<body>
	...
	<form action="<?php echo U('redman/add');?>" method="post" enctype="multipart/form-data">
		<p>商品名称</p>
		<input type="text" name="redman_name"><br>
		<p>父级分类</p>
		<select name="parent_id">
			<option value="0">没有父级</option>
			<?php if(is_array($first_menu_list)): $i = 0; $__LIST__ = $first_menu_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><option value="<?php echo ($value['redman_id']); ?>"><?php echo ($value['redman_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		<p>商品图片</p>
		<input type="file" class="dropify" name="redman_path"><br>
		<p>作者名称</p>
		<input type="text" name="redman_author"><br>
		<p>作者头像</p>
		<input type="file" class="dropify" name="redman_thumb"><br>
		<a href="<?php echo U('redman/add');?>"><input type="submit" class="btn" value="添加"></a>
	</form>
		</div>
	</div>
